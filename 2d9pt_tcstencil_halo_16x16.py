...
block_size_n=16
r = 2
inner_m = 512 #输出网格内部点大小
inner_n = 512
a = torch.ones((inner_m+2*r, inner_n+2*r), device="cuda", dtype=torch.float16)
# a = torch.randn((inner_m+2*r, inner_n+2*r), device="cuda", dtype=torch.float16)
b = torch.zeros((inner_m+2*r, inner_n+2*r), device=a.device, dtype=torch.float16)
